...
bool loadCameraFromFile(const std::string& path, Camera& camera, std::string& error) {
    ...
    if (!isFinite(loaded.n) || !isFinite(loaded.v) || !isFinite(loaded.c) ||
        !std::isfinite(loaded.d) || !std::isfinite(loaded.hx) ||
        !std::isfinite(loaded.hy)) {
        error = "os parâmetros da câmera devem ser finitos em '" + path + "'";
        return false;
    }

    const float nLength = loaded.n.length();
    const float vLength = loaded.v.length();
    if (!std::isfinite(nLength) || !std::isfinite(vLength) || nLength == 0.0F ||
        vLength == 0.0F) {
        error = "vetor N ou V nulo ou não finito em '" + path + "'";
        return false;
    }
    ...
    const float orthogonalLength = orthogonal.length();
    if (!std::isfinite(orthogonalLength) || orthogonalLength == 0.0F) {
